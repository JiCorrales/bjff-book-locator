...
CREATE TABLE IF NOT EXISTS `bjff_book_locator`.`Shelves` (
  ...
  `image_path` VARCHAR(255) NULL DEFAULT NULL COMMENT 'Path to shelf visualization image',
  `is_active` TINYINT NULL DEFAULT '1',
  `is_deleted` TINYINT NULL DEFAULT '0',
  `created_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX `idx_shelves_image_path` (`image_path` ASC) VISIBLE,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bjff_book_locator`.`vw_complete_structure` (`module_id` INT, `module_number` INT, `module_name` INT, `module_part_id` INT, `part_identifier` INT, `part_order` INT, `shelving_unit_id` INT, `unit_identifier` INT, `unit_order` INT, `shelf_id` INT, `shelf_order` INT, `shelf_image_path` INT, `shelf_range_start` INT, `shelf_range_end` INT, `shelf_key_start` INT, `shelf_key_end` INT, `location_text` INT, `is_active` INT, `is_deleted` INT);
...
USE `bjff_book_locator`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `bjff_book_locator`.`vw_complete_structure` AS select `m`.`module_id` AS `module_id`,`m`.`module_number` AS `module_number`,`m`.`module_name` AS `module_name`,`mp`.`module_part_id` AS `module_part_id`,`mp`.`part_name` AS `part_identifier`,`mp`.`part_number` AS `part_order`,`u`.`shelving_unit_id` AS `shelving_unit_id`,`u`.`unit_name` AS `unit_identifier`,`u`.`unit_number` AS `unit_order`,`s`.`shelf_id` AS `shelf_id`,`s`.`shelf_number` AS `shelf_order`,`s`.`image_path` AS `shelf_image_path`,`s`.`range_start` AS `shelf_range_start`,`s`.`range_end` AS `shelf_range_end`,`s`.`key_start` AS `shelf_key_start`,`s`.`key_end` AS `shelf_key_end`,concat('Module ',coalesce(`m`.`module_number`,'?'),' - ',coalesce(`mp`.`part_name`,'?'),' - Unit ',coalesce(`u`.`unit_name`,`u`.`unit_number`),' - Shelf ',coalesce(`s`.`shelf_number`,'?')) AS `location_text`,`s`.`is_active` AS `is_active`,`s`.`is_deleted` AS `is_deleted` from (((`bjff_book_locator`.`modules` `m` join `bjff_book_locator`.`module_parts` `mp` on((`mp`.`module_id` = `m`.`module_id`))) join `bjff_book_locator`.`shelving_units` `u` on((`u`.`module_part_id` = `mp`.`module_part_id`))) join `bjff_book_locator`.`shelves` `s` on((`s`.`shelving_unit_id` = `u`.`shelving_unit_id`))) where ((`m`.`is_deleted` = 0) and (`mp`.`is_deleted` = 0) and (`s`.`is_deleted` = 0)) order by `m`.`module_number`,`mp`.`part_number`,`u`.`unit_number`,`s`.`shelf_number`;
